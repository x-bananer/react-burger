...
const BurgerIngredients: React.FC<BurgerIngredientsProps> = ({ className }) => {
    const { items: ingredients, isLoading, isError } = useSelector((state: any) => state.ingredients);

    const [activeTab, setActiveTab] = useState('Булки');

    const sectionsRef = useRef<{ [key: string]: HTMLDivElement | null }>({ Булки: null, Соусы: null, Начинки: null });
    const isProgrammaticScroll = useRef<boolean>(false);

    const buns = useMemo(() => ingredients.filter((i: any) => i.type === 'bun'), [ingredients]);
    const sauces = useMemo(() => ingredients.filter((i: any) => i.type === 'sauce'), [ingredients]);
    const maines = useMemo(() => ingredients.filter((i: any) => i.type === 'main'), [ingredients]);
